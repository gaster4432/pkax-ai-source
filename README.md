...
- **MCP tools**: loaded from `%APPDATA%\Pkax\config\config.jsonc` only. Tool names are namespaced `server_tool`. Tool calls render as expandable cards in chat.
...
2. `npm start` to launch. On first run the app asks for your Cloudflare **Account ID** and **API
   token** (Workers AI) and saves them to `%APPDATA%\Pkax\config\cloudflare.json`. Models default
   to `@cf/qwen/qwen3.8-27b` / `@cf/black-forest-labs/flux-1-schnell`.

## Data locations

Everything lives under `%APPDATA%\Pkax` (created and seeded on first run from the project files):

| Path | Purpose |
|---|---|
| `%APPDATA%\Pkax\store.json` | Conversations + settings |
| `%APPDATA%\Pkax\mods\` | Installed mods |
| `%APPDATA%\Pkax\mod-storage\<modId>\` | Per-mod sandboxed storage |
| `%APPDATA%\Pkax\mod-configs\<modId>.json` | Per-mod settings |
| `%APPDATA%\Pkax\config\config.jsonc` | MCP server configuration (seeded from project) |
| `%APPDATA%\Pkax\config\credentials.json` | MCP OAuth tokens (seeded from project) |
| `%APPDATA%\Pkax\config\cloudflare.json` | Cloudflare Account ID + API token + model prefs |

MCP servers are loaded **only** from `%APPDATA%\Pkax\config\config.jsonc` — no opencode or
...
electron/appdata.js      AppData layout (%APPDATA%\Pkax): mods/, config/, seeding
...
- Credentials live in `%APPDATA%\Pkax\config\cloudflare.json` (gitignored via AppData); the token never enters the renderer.
